                        ...
                        end=max(index(qsim)),...)
{
 ...
 # ylim2 <- c(max(c(2*rain),na.rm=TRUE),min(Qr, 0, na.rm=T))
  ...
  if(disp.pos)
  {
    disp.sim.time(tm, label=F, lty=1, lwd=1)
  }
...
#   par("cex.lab"=cex)
  ...
  # line showing current time
	if(disp.pos)
	{
	    disp.sim.time(tm, fmt=par$time.pos.fmt, lwd=1, lty=1)
	}
	# U-shaped borders
  box(bty="u")
                      ...
                      ...)
{
  ...
  plot.zoo(rain,
	...
	if(axes)
	{
		...
		axis(side=side, at=at, labels=at, cex.axis=cex.axis)
