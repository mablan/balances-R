...
combine.groupings.2 <- function(dem,
                                ...
                                remove.areas=TRUE,
                                renumber=FALSE
) # if T then groups are strictly equal in plan area
{
  if(is.null(catch))
...
# determine values given layer within the classification
get_layer_values <- function(cm, layer.num)
{
  # magnitude of values in terms of powers of 10
  #  pows <- round(log10(cm))
  
  ilayer <- which(pows==layer.num)
  
  vals <- cm-10^layer.num
