         ...
         fact=1)
{
  ...
  {
    # data.frame(matrix(ncol=length(fields), nrow=0, byrow=T))
    return(NULL)
  }
...
read.obs <- function(fn,
                          ...
                          #  end=NULL,
                            ...
                            header=TRUE, #is.character(tm) | is.character(val),           # whether column names are present, can also point to a line in the input
