...
find_time_interval <- function(tm, ser, as.index=F, warn=T)
{
	if(is.zoo(ser))
	{
...
# # find the requitred no. maxima in teh given series
find_maxima <- function(x, n=2, nwin=4)
{
  # divide the period into given nuumber of windows, find the maximum in each and 
  # 
  

  lo <- loess(as.numeric(x)~as.numeric(index(x)))

  # lengths of the gadrient of the fitted graph
  grad.sgns <- rle(sign(diff(lo$fitted)))

  imax <- which(diff(grad.sgns$values)<0)

  # woork out which values these corresponds to
  iminmax <- (cumsum(grad.sgns$lengths)+1)[imax]
  ...
  # returning a list whose first elements are the
