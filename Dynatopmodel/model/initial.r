...
#    }
  ...
  # any drainage params (custom exponential profile)
...
{
  if(length(x) > 0 && max(x, na.rm = T) > max)
  {
    message(msg)
    x <- x/fact
  }
...
#  LogEvent("Dynamic TOPMODEL run started")
  ...
  if(length(qobs)>0)
  {
		...
		qin <- sum(rain[,1], na.rm = TRUE)        #qin <- sum(rain)
    qout <- sum(qobs[,1])
